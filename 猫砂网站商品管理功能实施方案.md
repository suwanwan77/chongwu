# 猫砂网站商品管理功能实施方案

## 📋 项目概述

为Pawganic猫砂电商网站添加完整的商品管理功能，包括前端商品展示、购物车、订单管理，以及后台管理系统的商品管理模块。

## 🏗️ 系统架构

### 当前技术栈
- **前端网站**: WordPress静态HTML页面 (运行在 http://localhost:8080)
- **后台管理**: Vue3 + Element Plus (运行在 http://localhost:80)
- **后端API**: NestJS + Prisma + MySQL (运行在 http://localhost:3000)
- **数据库**: MySQL (meimei-prisma)
- **缓存**: Redis

### 架构设计原则
✅ 前端商品展示与后台商品管理分离
✅ 使用独立的商品相关数据表
✅ 创建独立的API模块 `product-management`
✅ 支持商品分类、库存、价格管理
✅ 支持购物车和订单功能

## 📁 现有页面分析

### 前端页面（共21个HTML文件）

#### 1. 商品相关页面（4个）
- **`frontend/shop/index.html`** - 商品列表页（主要商品展示页）
- **`frontend/shop/-canvas_4_grid.html`** - 商品网格视图
- **`frontend/shop/-canvas_4_list.html`** - 商品列表视图
- **`frontend/shop/intelligent-marble-knife/index.html`** - 商品详情页示例

#### 2. 购物流程页面（3个）
- **`frontend/cart/index.html`** - 购物车页面
- **`frontend/order-received/index.html`** - 订单确认页面
- **`frontend/wishlist/index.html`** - 心愿单页面

#### 3. 用户相关页面（4个）
- **`frontend/my-account/index.html`** - 登录/我的账户
- **`frontend/register/index.html`** - 注册页面
- **`frontend/Personal-Center/index.html`** - 个人中心
- **`frontend/my-account/lost-password/index.html`** - 找回密码

#### 4. 信息页面（10个）
- **`frontend/index.html`** - 首页（主页）
- **`frontend/index1.html`** - 备用首页
- **`frontend/about-us/index.html`** - 关于我们
- **`frontend/contact-us/index.html`** - 联系我们
- **`frontend/charity/index.html`** - 慈善页面
- **`frontend/pricing/index.html`** - 价格页面
- **`frontend/faq/index.html`** - 常见问题
- **`frontend/senior-pets-how-much-to-feed-your-pet/index.html`** - 宠物喂养指南
- **`frontend/understanding-pet-food-labels/index.html`** - 宠物食品标签说明

### 现有数据结构分析

从HTML页面中提取的商品数据结构：

```javascript
// 商品对象示例（从 shop/intelligent-marble-knife/index.html 提取）
{
  productId: 58,
  productName: "Intelligent Marble Knife",
  sku: "PT14Z61",
  price: 132.93,
  originalPrice: 134.27,
  discount: "-1%",
  mainImage: "../../wp-content/uploads/2021/11/16.jpg",
  images: [
    "../../wp-content/uploads/2021/11/16.jpg",
    "../../wp-content/uploads/2021/11/9.jpg",
    "../../wp-content/uploads/2021/11/4.jpg",
    "../../wp-content/uploads/2021/11/12.jpg"
  ],
  shortDescription: "Ipsam rerum modi eius impedit inventore doloribus non et. Sit consequuntur et dolore iure ducimus beatae quis.",
  availability: "In Stock",
  categories: ["Dry Food", "Uncategorized"],
  // 注意：不包含品牌信息
}

// 购物车项目（从 cart/index.html 提取）
{
  cartId: 1,
  customerId: 1,
  productId: 58,
  productName: "Intelligent Marble Knife",
  productImage: "../../wp-content/uploads/2021/11/16.jpg",
  quantity: 1,
  price: 132.93,
  subtotal: 132.93,
  stock: 100
}

// 订单项目（从 order-received/index.html 提取）
{
  orderId: 1,
  orderNo: "ORD20251106001",
  customerId: 1,
  items: [
    {
      itemId: 1,
      productId: 58,
      productName: "Intelligent Marble Knife",
      productImage: "../../wp-content/uploads/2021/11/16-150x150.jpg",
      quantity: 2,
      price: 132.93,
      subtotal: 265.86
    }
  ],
  totalAmount: 265.86,
  shippingFee: 0,
  payAmount: 265.86,
  status: "0",  // 0-待支付
  shippingAddress: {
    receiverName: "张三",
    receiverPhone: "13800138000",
    province: "北京市",
    city: "北京市",
    district: "朝阳区",
    detailAddress: "某某街道123号",
    postalCode: "100000"
  },
  createTime: "2025-11-06 10:30:00"
}
```

## 🗄️ 数据库设计

本方案共设计 **7张数据表**：

1. **product** - 商品表（核心表）
2. **product_category** - 商品分类表（支持多级分类）
3. **shopping_cart** - 购物车表
4. **customer_order** - 订单表
5. **order_item** - 订单明细表
6. **wishlist** - 心愿单表
7. **customer_address** - 收货地址表（新增）

---

### 1. 商品表 (product)

```sql
CREATE TABLE `product` (
  `productId` INT NOT NULL AUTO_INCREMENT COMMENT '商品ID',
  `productName` VARCHAR(200) NOT NULL COMMENT '商品名称',
  `productCode` VARCHAR(100) DEFAULT NULL COMMENT '商品编码(SKU)',
  `categoryId` INT DEFAULT NULL COMMENT '分类ID',
  `price` DECIMAL(10,2) NOT NULL COMMENT '售价',
  `originalPrice` DECIMAL(10,2) DEFAULT NULL COMMENT '原价',
  `costPrice` DECIMAL(10,2) DEFAULT NULL COMMENT '成本价',
  `stock` INT DEFAULT 0 COMMENT '库存数量',
  `unit` VARCHAR(20) DEFAULT '件' COMMENT '单位',
  `weight` DECIMAL(10,2) DEFAULT NULL COMMENT '重量(kg)',
  `volume` DECIMAL(10,2) DEFAULT NULL COMMENT '体积(m³)',
  `mainImage` TEXT DEFAULT NULL COMMENT '主图(JSON对象，包含original/large/medium/small/thumbnail/webp等多尺寸路径)',
  `images` TEXT DEFAULT NULL COMMENT '商品图片(JSON数组，每个元素是包含多尺寸路径的对象)',
  `shortDescription` VARCHAR(500) DEFAULT NULL COMMENT '简短描述',
  `description` TEXT DEFAULT NULL COMMENT '详细描述',
  `specifications` TEXT DEFAULT NULL COMMENT '规格参数(JSON)',
  `tags` VARCHAR(500) DEFAULT NULL COMMENT '标签',
  `status` CHAR(1) DEFAULT '0' COMMENT '状态：0-上架，1-下架',
  `isFeatured` TINYINT DEFAULT 0 COMMENT '是否精选：0-否，1-是',
  `isNew` TINYINT DEFAULT 0 COMMENT '是否新品：0-否，1-是',
  `isHot` TINYINT DEFAULT 0 COMMENT '是否热销：0-否，1-是',
  `displayPosition` VARCHAR(100) DEFAULT NULL COMMENT '展示位置：home_featured(首页精选),shop_list(商品列表),product_related(相关商品),多个用逗号分隔',
  `displayOrder` INT DEFAULT 0 COMMENT '展示顺序（数字越小越靠前）',
  `viewCount` INT DEFAULT 0 COMMENT '浏览次数',
  `salesCount` INT DEFAULT 0 COMMENT '销售数量',
  `rating` DECIMAL(3,2) DEFAULT 0.00 COMMENT '评分(0-5)',
  `reviewCount` INT DEFAULT 0 COMMENT '评论数量',
  `sort` INT DEFAULT 0 COMMENT '排序',
  `delFlag` CHAR(1) DEFAULT '0' COMMENT '删除标志：0-未删除，1-已删除',
  `createBy` VARCHAR(64) DEFAULT NULL COMMENT '创建者',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateBy` VARCHAR(64) DEFAULT NULL COMMENT '更新者',
  `updateTime` DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `remark` VARCHAR(500) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`productId`),
  UNIQUE KEY `product_productCode_key` (`productCode`),
  KEY `idx_product_categoryId` (`categoryId`),
  KEY `idx_product_status` (`status`),
  KEY `idx_product_createTime` (`createTime`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='商品表';
```

### 2. 商品分类表 (product_category)

```sql
CREATE TABLE `product_category` (
  `categoryId` INT NOT NULL AUTO_INCREMENT COMMENT '分类ID',
  `parentId` INT DEFAULT 0 COMMENT '父分类ID（0表示顶级分类）',
  `categoryName` VARCHAR(100) NOT NULL COMMENT '分类名称',
  `categoryCode` VARCHAR(50) DEFAULT NULL COMMENT '分类编码',
  `icon` VARCHAR(200) DEFAULT NULL COMMENT '分类图标',
  `image` VARCHAR(500) DEFAULT NULL COMMENT '分类图片',
  `sort` INT DEFAULT 0 COMMENT '排序',
  `level` INT DEFAULT 1 COMMENT '层级：1-一级，2-二级，3-三级',
  `status` CHAR(1) DEFAULT '0' COMMENT '状态：0-正常，1-停用',
  `delFlag` CHAR(1) DEFAULT '0' COMMENT '删除标志：0-未删除，1-已删除',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateTime` DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `remark` VARCHAR(500) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`categoryId`),
  KEY `idx_category_parentId` (`parentId`),
  KEY `idx_category_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='商品分类表';
```

### 3. 购物车表 (shopping_cart)

```sql
CREATE TABLE `shopping_cart` (
  `cartId` INT NOT NULL AUTO_INCREMENT COMMENT '购物车ID',
  `customerId` INT NOT NULL COMMENT '用户ID',
  `productId` INT NOT NULL COMMENT '商品ID',
  `quantity` INT NOT NULL DEFAULT 1 COMMENT '数量',
  `price` DECIMAL(10,2) NOT NULL COMMENT '加入购物车时的价格',
  `selected` TINYINT DEFAULT 1 COMMENT '是否选中：0-未选中，1-选中',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateTime` DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`cartId`),
  KEY `idx_cart_customerId` (`customerId`),
  KEY `idx_cart_productId` (`productId`),
  UNIQUE KEY `cart_customer_product_key` (`customerId`, `productId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='购物车表';
```

### 4. 订单表 (customer_order)

```sql
CREATE TABLE `customer_order` (
  `orderId` INT NOT NULL AUTO_INCREMENT COMMENT '订单ID',
  `orderNo` VARCHAR(50) NOT NULL COMMENT '订单号',
  `customerId` INT NOT NULL COMMENT '用户ID',
  `totalAmount` DECIMAL(10,2) NOT NULL COMMENT '订单总金额',
  `discountAmount` DECIMAL(10,2) DEFAULT 0.00 COMMENT '优惠金额',
  `shippingFee` DECIMAL(10,2) DEFAULT 0.00 COMMENT '运费',
  `payAmount` DECIMAL(10,2) NOT NULL COMMENT '实付金额',
  `payMethod` VARCHAR(20) DEFAULT NULL COMMENT '支付方式',
  `payTime` DATETIME DEFAULT NULL COMMENT '支付时间',
  `status` CHAR(1) DEFAULT '0' COMMENT '订单状态：0-待支付，1-已支付，2-已发货，3-已完成，4-已取消',
  `shippingAddress` TEXT DEFAULT NULL COMMENT '收货地址(JSON)',
  `remark` VARCHAR(500) DEFAULT NULL COMMENT '订单备注',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateTime` DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`orderId`),
  UNIQUE KEY `order_orderNo_key` (`orderNo`),
  KEY `idx_order_customerId` (`customerId`),
  KEY `idx_order_status` (`status`),
  KEY `idx_order_createTime` (`createTime`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='订单表';
```

### 5. 订单明细表 (order_item)

```sql
CREATE TABLE `order_item` (
  `itemId` INT NOT NULL AUTO_INCREMENT COMMENT '订单明细ID',
  `orderId` INT NOT NULL COMMENT '订单ID',
  `productId` INT NOT NULL COMMENT '商品ID',
  `productName` VARCHAR(200) NOT NULL COMMENT '商品名称',
  `productImage` VARCHAR(500) DEFAULT NULL COMMENT '商品图片',
  `price` DECIMAL(10,2) NOT NULL COMMENT '商品单价',
  `quantity` INT NOT NULL COMMENT '购买数量',
  `subtotal` DECIMAL(10,2) NOT NULL COMMENT '小计金额',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`itemId`),
  KEY `idx_orderItem_orderId` (`orderId`),
  KEY `idx_orderItem_productId` (`productId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='订单明细表';
```

### 6. 心愿单表 (wishlist)

```sql
CREATE TABLE `wishlist` (
  `wishlistId` INT NOT NULL AUTO_INCREMENT COMMENT '心愿单ID',
  `customerId` INT NOT NULL COMMENT '用户ID',
  `productId` INT NOT NULL COMMENT '商品ID',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`wishlistId`),
  KEY `idx_wishlist_customerId` (`customerId`),
  KEY `idx_wishlist_productId` (`productId`),
  UNIQUE KEY `wishlist_customer_product_key` (`customerId`, `productId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='心愿单表';
```

### 7. 收货地址表 (customer_address)

**用途**：存储用户的收货地址信息（新西兰地址格式）

```sql
CREATE TABLE `customer_address` (
  `addressId` INT NOT NULL AUTO_INCREMENT COMMENT '地址ID',
  `customerId` INT NOT NULL COMMENT '用户ID',
  `receiverName` VARCHAR(100) NOT NULL COMMENT '收货人姓名',
  `receiverPhone` VARCHAR(20) NOT NULL COMMENT '收货人电话',
  `streetAddress` VARCHAR(200) NOT NULL COMMENT '街道地址（Street Address）',
  `suburb` VARCHAR(100) DEFAULT NULL COMMENT '郊区/区域（Suburb）',
  `city` VARCHAR(100) NOT NULL COMMENT '城市（City）',
  `region` VARCHAR(100) DEFAULT NULL COMMENT '地区（Region）- 如Auckland, Wellington等',
  `postalCode` VARCHAR(10) NOT NULL COMMENT '邮政编码（Postcode）',
  `country` VARCHAR(50) DEFAULT 'New Zealand' COMMENT '国家',
  `isDefault` TINYINT DEFAULT 0 COMMENT '是否默认地址：0-否，1-是',
  `addressTag` VARCHAR(20) DEFAULT NULL COMMENT '地址标签：Home、Work等',
  `deliveryInstructions` TEXT DEFAULT NULL COMMENT '配送说明（可选）',
  `delFlag` CHAR(1) DEFAULT '0' COMMENT '删除标志：0-未删除，1-已删除',
  `createTime` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updateTime` DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`addressId`),
  KEY `idx_address_customerId` (`customerId`),
  KEY `idx_address_isDefault` (`isDefault`),
  KEY `idx_address_city` (`city`),
  KEY `idx_address_postalCode` (`postalCode`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='收货地址表（新西兰格式）';
```

**新西兰地址格式说明**：
- **Street Address**：街道地址，如 "123 Queen Street"
- **Suburb**：郊区/区域，如 "Ponsonby", "Mount Eden"
- **City**：城市，如 "Auckland", "Wellington", "Christchurch"
- **Region**：地区（可选），如 "Auckland Region", "Wellington Region"
- **Postcode**：邮政编码，4位数字，如 "1010", "6011"
- **Delivery Instructions**：配送说明（可选），如 "Leave at front door", "Ring doorbell"

**示例地址**：
```
John Smith
021 123 4567
123 Queen Street
Ponsonby
Auckland
Auckland Region
1011
New Zealand
```

## 🔌 后端API接口设计

### API路由前缀

- **前端商品API**: `/api/frontend/product`
- **前端用户API**: `/api/frontend/customer`
- **后台管理API**: `/system/product`

### 认证机制

#### 前端用户认证
- **使用守卫**: `CustomerJwtAuthGuard`（已存在于项目中）
- **Token类型**: JWT Bearer Token
- **Token存储**: Redis（key: `customer_token:{customerId}`）
- **Token有效期**: 7天（可在config中配置）
- **Token payload**: `{ customerId, email, type: 'customer' }`

#### 后台管理员认证
- **使用守卫**: `JwtAuthGuard`（已存在于项目中）
- **Token类型**: JWT Bearer Token
- **Token存储**: Redis（key: `USER_TOKEN_KEY:{userId}`）
- **Token有效期**: 1年（由Redis管控实际过期时间）
- **Token payload**: `{ userId, pv }`（pv为密码版本号）

### 前端商品API接口

#### 1. 获取商品列表（分页+筛选）
```typescript
GET /api/frontend/product/list
Query参数:
  - pageNum: number (页码，默认1)
  - pageSize: number (每页数量，默认12)
  - categoryId?: number (分类ID)
  - keyword?: string (搜索关键词)
  - minPrice?: number (最低价格)
  - maxPrice?: number (最高价格)
  - sortBy?: string (排序字段：price/sales/rating/createTime)
  - sortOrder?: string (排序方式：asc/desc)
  - isFeatured?: boolean (是否精选)
  - isNew?: boolean (是否新品)
  - isHot?: boolean (是否热销)

Response:
{
  "code": 200,
  "msg": "成功",
  "data": {
    "rows": [
      {
        "productId": 58,
        "productName": "Intelligent Marble Knife",
        "price": 132.93,
        "originalPrice": 221.55,
        "mainImage": "/uploads/2021/11/16.jpg",
        "shortDescription": "高品质猫砂产品",
        "stock": 100,
        "rating": 4.5,
        "reviewCount": 23,
        "salesCount": 156,
        "isFeatured": true,
        "isNew": false,
        "isHot": true
      }
    ],
    "total": 100
  }
}
```

#### 2. 获取商品详情
```typescript
GET /api/frontend/product/:productId

Response:
{
  "code": 200,
  "msg": "成功",
  "data": {
    "productId": 58,
    "productName": "Intelligent Marble Knife",
    "productCode": "PT14Z61",
    "categoryId": 1,
    "categoryName": "猫砂",
    "price": 132.93,
    "originalPrice": 221.55,
    "stock": 100,
    "unit": "袋",
    "weight": 5.0,
    "mainImage": "/uploads/2021/11/16.jpg",
    "images": [
      "/uploads/2021/11/16.jpg",
      "/uploads/2021/11/16-1.jpg"
    ],
    "shortDescription": "高品质猫砂产品",
    "description": "<p>详细的商品描述HTML内容</p>",
    "specifications": {
      "材质": "天然豆腐砂",
      "颗粒大小": "2.5mm"
    },
    "tags": "环保,无尘,可冲厕",
    "rating": 4.5,
    "reviewCount": 23,
    "salesCount": 156
  }
}
```

#### 3. 获取商品分类树
```typescript
GET /api/frontend/product/category/tree

Response:
{
  "code": 200,
  "msg": "成功",
  "data": [
    {
      "categoryId": 1,
      "categoryName": "猫砂",
      "children": [
        {
          "categoryId": 11,
          "categoryName": "豆腐猫砂"
        }
      ]
    }
  ]
}
```

#### 4. 添加到购物车
```typescript
POST /api/frontend/product/cart/add
Headers:
  Authorization: Bearer {token}

Request Body:
{
  "productId": 58,
  "quantity": 2
}

Response:
{
  "code": 200,
  "msg": "添加成功"
}
```

#### 5. 获取购物车列表
```typescript
GET /api/frontend/product/cart/list
Headers:
  Authorization: Bearer {token}

Response:
{
  "code": 200,
  "msg": "成功",
  "data": [
    {
      "cartId": 1,
      "productId": 58,
      "productName": "Intelligent Marble Knife",
      "productImage": "/uploads/2021/11/16.jpg",
      "price": 132.93,
      "quantity": 2,
      "subtotal": 265.86,
      "stock": 100
    }
  ]
}
```

#### 6. 获取收货地址列表
```typescript
GET /api/frontend/customer/address/list
Headers:
  Authorization: Bearer {token}

Response:
{
  "code": 200,
  "msg": "成功",
  "data": [
    {
      "addressId": 1,
      "receiverName": "John Smith",
      "receiverPhone": "021 123 4567",
      "streetAddress": "123 Queen Street",
      "suburb": "Ponsonby",
      "city": "Auckland",
      "region": "Auckland Region",
      "postalCode": "1011",
      "country": "New Zealand",
      "isDefault": 1,
      "addressTag": "Home",
      "deliveryInstructions": "Leave at front door"
    }
  ]
}
```

#### 7. 添加/修改收货地址
```typescript
POST /api/frontend/customer/address
Headers:
  Authorization: Bearer {token}

Request Body:
{
  "addressId": 1,  // 修改时传入，新增时不传
  "receiverName": "John Smith",
  "receiverPhone": "021 123 4567",
  "streetAddress": "123 Queen Street",
  "suburb": "Ponsonby",
  "city": "Auckland",
  "region": "Auckland Region",
  "postalCode": "1011",
  "country": "New Zealand",
  "isDefault": 1,
  "addressTag": "Home",
  "deliveryInstructions": "Leave at front door"
}

Response:
{
  "code": 200,
  "msg": "保存成功"
}
```

#### 8. 删除收货地址
```typescript
DELETE /api/frontend/customer/address/:addressId
Headers:
  Authorization: Bearer {token}

Response:
{
  "code": 200,
  "msg": "删除成功"
}
```

#### 9. 创建订单
```typescript
POST /api/frontend/product/order/create
Headers:
  Authorization: Bearer {token}

Request Body:
{
  "cartIds": [1, 2, 3],
  "addressId": 1,  // 使用已保存的地址ID
  "remark": "请尽快发货"
}

Response:
{
  "code": 200,
  "msg": "订单创建成功",
  "data": {
    "orderId": 1,
    "orderNo": "ORD20251106001",
    "payAmount": 531.72
  }
}
```

### 后台管理API接口

#### 1. 获取商品列表（后台）
```typescript
GET /system/product/list
Headers:
  Authorization: Bearer {admin_token}
Query参数:
  - pageNum: number
  - pageSize: number
  - productName?: string
  - categoryId?: number
  - status?: string

Response:
{
  "code": 200,
  "msg": "成功",
  "rows": [...],
  "total": 100
}
```

#### 2. 新增商品
```typescript
POST /system/product
Headers:
  Authorization: Bearer {admin_token}

Request Body:
{
  "productName": "新商品名称",
  "productCode": "PRD-2024-002",
  "categoryId": 1,
  "price": 99.99,
  "stock": 100,
  "mainImage": "/uploads/product/main.jpg",
  "shortDescription": "简短描述",
  "status": "0"
}

Response:
{
  "code": 200,
  "msg": "新增成功"
}
```

#### 3. 修改商品
```typescript
PUT /system/product
Headers:
  Authorization: Bearer {admin_token}

Request Body:
{
  "productId": 58,
  "productName": "修改后的商品名称"
}

Response:
{
  "code": 200,
  "msg": "修改成功"
}
```

#### 4. 删除商品
```typescript
DELETE /system/product/:productId
Headers:
  Authorization: Bearer {admin_token}

Response:
{
  "code": 200,
  "msg": "删除成功"
}
```

#### 5. 批量更新商品位置
```typescript
POST /system/product/position/batch
Headers:
  Authorization: Bearer {admin_token}

Request Body:
{
  "positions": [
    {
      "productId": 58,
      "displayPosition": "home_featured,shop_list",  // 多个位置用逗号分隔
      "displayOrder": 1
    },
    {
      "productId": 56,
      "displayPosition": "home_featured,shop_list",
      "displayOrder": 2
    }
  ]
}

Response:
{
  "code": 200,
  "msg": "位置更新成功"
}
```

#### 6. 获取指定位置的商品列表
```typescript
GET /system/product/position/:position
Headers:
  Authorization: Bearer {admin_token}
Query参数:
  - position: string (home_featured | shop_list | product_related)

Response:
{
  "code": 200,
  "msg": "成功",
  "data": [
    {
      "productId": 58,
      "productName": "Intelligent Marble Knife",
      "mainImage": "/uploads/2021/11/16.jpg",
      "price": 132.93,
      "displayOrder": 1
    }
  ]
}
```

## � Prisma Schema定义

需要在 `backend/meimei-prisma-vue3/meimei-admin/prisma/schema.prisma` 文件中添加以下模型定义：

```prisma
/// 商品表
model Product {
  productId         Int       @id @default(autoincrement())
  productName       String    @db.VarChar(200)
  productCode       String    @unique @db.VarChar(50)
  categoryId        Int?
  price             Decimal   @db.Decimal(10, 2)
  originalPrice     Decimal?  @db.Decimal(10, 2)
  costPrice         Decimal?  @db.Decimal(10, 2)
  stock             Int       @default(0)
  unit              String?   @default("件") @db.VarChar(20)
  weight            Decimal?  @db.Decimal(10, 2)
  volume            Decimal?  @db.Decimal(10, 2)
  mainImage         String?   @db.Text  // JSON对象：{original, large, medium, small, thumbnail, webp}
  images            String?   @db.Text  // JSON数组：[{original, large, medium, small, thumbnail, webp}, ...]
  shortDescription  String?   @db.VarChar(500)
  description       String?   @db.Text
  specifications    String?   @db.Text
  tags              String?   @db.VarChar(200)
  salesCount        Int       @default(0)
  viewCount         Int       @default(0)
  rating            Decimal?  @default(0) @db.Decimal(3, 2)
  reviewCount       Int       @default(0)
  isFeatured        Boolean   @default(false)
  isNew             Boolean   @default(false)
  isHot             Boolean   @default(false)
  displayPosition   String?   @db.VarChar(100)
  displayOrder      Int       @default(0)
  status            String    @default("0") @db.Char(1)
  delFlag           String    @default("0") @db.Char(1)
  createBy          String?   @db.VarChar(64)
  createTime        DateTime? @default(now())
  updateBy          String?   @db.VarChar(64)
  updateTime        DateTime? @updatedAt
  remark            String?   @db.VarChar(500)

  category          ProductCategory? @relation(fields: [categoryId], references: [categoryId])
  cartItems         ShoppingCart[]
  orderItems        OrderItem[]
  wishlistItems     Wishlist[]

  @@index([categoryId])
  @@index([status])
  @@index([displayPosition])
  @@index([displayOrder])
  @@map("product")
}

/// 商品分类表
model ProductCategory {
  categoryId     Int       @id @default(autoincrement())
  categoryName   String    @db.VarChar(100)
  parentId       Int?      @default(0)
  orderNum       Int       @default(0)
  icon           String?   @db.VarChar(200)
  status         String    @default("0") @db.Char(1)
  delFlag        String    @default("0") @db.Char(1)
  createBy       String?   @db.VarChar(64)
  createTime     DateTime? @default(now())
  updateBy       String?   @db.VarChar(64)
  updateTime     DateTime? @updatedAt
  remark         String?   @db.VarChar(500)

  products       Product[]

  @@index([parentId])
  @@index([status])
  @@map("product_category")
}

/// 购物车表
model ShoppingCart {
  cartId         Int       @id @default(autoincrement())
  customerId     Int
  productId      Int
  quantity       Int       @default(1)
  createTime     DateTime? @default(now())
  updateTime     DateTime? @updatedAt

  customer       Customer  @relation(fields: [customerId], references: [customerId])
  product        Product   @relation(fields: [productId], references: [productId])

  @@index([customerId])
  @@index([productId])
  @@map("shopping_cart")
}

/// 订单表
model CustomerOrder {
  orderId          Int       @id @default(autoincrement())
  orderNo          String    @unique @db.VarChar(50)
  customerId       Int
  totalAmount      Decimal   @db.Decimal(10, 2)
  shippingFee      Decimal   @default(0) @db.Decimal(10, 2)
  payAmount        Decimal   @db.Decimal(10, 2)
  payMethod        String?   @db.VarChar(20)
  payTime          DateTime?
  shippingAddress  String    @db.Text
  status           String    @default("0") @db.Char(1)
  remark           String?   @db.VarChar(500)
  createTime       DateTime? @default(now())
  updateTime       DateTime? @updatedAt

  customer         Customer  @relation(fields: [customerId], references: [customerId])
  orderItems       OrderItem[]

  @@index([customerId])
  @@index([orderNo])
  @@index([status])
  @@map("customer_order")
}

/// 订单明细表
model OrderItem {
  itemId         Int       @id @default(autoincrement())
  orderId        Int
  productId      Int
  productName    String    @db.VarChar(200)
  productImage   String?   @db.VarChar(500)
  quantity       Int
  price          Decimal   @db.Decimal(10, 2)
  subtotal       Decimal   @db.Decimal(10, 2)

  order          CustomerOrder @relation(fields: [orderId], references: [orderId])
  product        Product       @relation(fields: [productId], references: [productId])

  @@index([orderId])
  @@index([productId])
  @@map("order_item")
}

/// 心愿单表
model Wishlist {
  wishlistId     Int       @id @default(autoincrement())
  customerId     Int
  productId      Int
  createTime     DateTime? @default(now())

  customer       Customer  @relation(fields: [customerId], references: [customerId])
  product        Product   @relation(fields: [productId], references: [productId])

  @@index([customerId])
  @@index([productId])
  @@map("wishlist")
}

/// 收货地址表（新西兰格式）
model CustomerAddress {
  addressId            Int       @id @default(autoincrement())
  customerId           Int
  receiverName         String    @db.VarChar(100)
  receiverPhone        String    @db.VarChar(20)
  streetAddress        String    @db.VarChar(200)
  suburb               String?   @db.VarChar(100)
  city                 String    @db.VarChar(100)
  region               String?   @db.VarChar(100)
  postalCode           String    @db.VarChar(10)
  country              String    @default("New Zealand") @db.VarChar(50)
  isDefault            Boolean   @default(false)
  addressTag           String?   @db.VarChar(20)
  deliveryInstructions String?   @db.Text
  delFlag              String    @default("0") @db.Char(1)
  createTime           DateTime? @default(now())
  updateTime           DateTime? @updatedAt

  customer             Customer  @relation(fields: [customerId], references: [customerId])

  @@index([customerId])
  @@index([isDefault])
  @@index([city])
  @@index([postalCode])
  @@map("customer_address")
}
```

**注意事项**：
- ✅ 所有模型都使用了 `@@map` 指定数据库表名（蛇形命名）
- ✅ 所有关联关系都已定义（一对多、多对一）
- ✅ 所有索引都已添加（提高查询性能）
- ✅ Customer模型已存在于项目中，需要添加关联关系：
  ```prisma
  model Customer {
    // ... 现有字段 ...
    cartItems     ShoppingCart[]
    orders        CustomerOrder[]
    wishlistItems Wishlist[]
    addresses     CustomerAddress[]
  }
  ```

## �💻 实施步骤

### 阶段一：数据库设计与创建（0.5天）

**任务清单：**
- [ ] 在Prisma Schema中添加7个新模型定义
- [ ] 在Customer模型中添加关联关系
- [ ] 运行 `npx prisma format` 格式化schema文件
- [ ] 运行 `npx prisma generate` 生成Prisma Client
- [ ] 运行 `npx prisma db push` 同步数据库结构（开发环境）
- [ ] 或运行 `npx prisma migrate dev --name add_product_tables` 创建迁移（生产环境推荐）
- [ ] 插入测试数据（商品分类、示例商品、收货地址等）
- [ ] 验证表结构和索引

### 阶段二：后端API开发（3-4天）

**任务清单：**
- [ ] 安装必要的依赖包
  - [ ] `npm install sharp uuid multer` - 图片处理和文件上传
  - [ ] `npm install @types/multer @types/uuid -D` - TypeScript类型定义
- [ ] 创建Prisma Schema定义（7张表）
- [ ] 生成Prisma Client
- [ ] 创建图片上传服务模块 ⭐ 新增
  - [ ] 创建 `upload.service.ts` - 处理文件上传
  - [ ] 创建 `image-processor.service.ts` - 使用sharp生成多尺寸图片
  - [ ] 配置文件上传拦截器（限制文件大小、类型）
  - [ ] 实现图片自动压缩和格式转换（WebP）
- [ ] 创建商品管理模块 (product-management)
  - [ ] 创建DTO（数据传输对象）
    - [ ] `CreateProductDto` - 包含图片验证
    - [ ] `UpdateProductDto` - 包含图片验证
    - [ ] `AddToCartDto` - 购物车DTO
    - [ ] `CreateOrderDto` - 订单DTO
    - [ ] `CreateAddressDto` - 地址DTO（新西兰格式验证）
  - [ ] 创建Service（业务逻辑层）
    - [ ] 实现数据隔离逻辑（customerId验证）⭐ 重要
    - [ ] 实现库存并发控制（事务处理）
  - [ ] 创建Controller（控制器层）
    - [ ] 从JWT token获取customerId（不从请求参数获取）⭐ 重要
  - [ ] 添加JWT认证守卫（复用现有的CustomerJwtAuthGuard）
- [ ] 实现前端商品API（9个核心接口）
  - [ ] 商品列表（支持分页、筛选、排序、位置筛选）
  - [ ] 商品详情
  - [ ] 分类树
  - [ ] 购物车增删改查（带customerId隔离）⭐ 重要
  - [ ] 收货地址增删改查（新西兰格式 + customerId隔离）⭐ 重要
  - [ ] 订单创建（带customerId隔离）⭐ 重要
- [ ] 实现后台管理API（6个核心接口）
  - [ ] 商品列表（后台）
  - [ ] 新增商品（含图片上传）
  - [ ] 修改商品（含图片更新）
  - [ ] 删除商品（软删除）
  - [ ] 批量更新商品位置 ⭐ 新增
  - [ ] 获取指定位置的商品列表 ⭐ 新增
- [ ] 编写单元测试
  - [ ] 测试数据隔离逻辑
  - [ ] 测试图片上传功能
  - [ ] 测试库存并发控制
- [ ] API文档生成（Swagger）

### 阶段三：前端商品展示（2-3天）

**任务清单：**
- [ ] 创建product.js服务文件（封装所有商品相关API调用）
- [ ] 修改首页 (index.html)
  - [ ] 从后端API获取首页精选商品（displayPosition=home_featured，最多4个）
  - [ ] 动态渲染Featured products区域的商品卡片
  - [ ] 实现"Add to cart"按钮功能
  - [ ] 实现"Add to wishlist"按钮功能
- [ ] 修改商品列表页面 (shop/index.html)
  - [ ] 从后端API获取商品列表数据（displayPosition=shop_list）
  - [ ] 动态渲染商品卡片
  - [ ] 实现"Add to cart"按钮功能（调用购物车API）
  - [ ] 实现"Add to wishlist"按钮功能
  - [ ] 实现商品分页加载
  - [ ] 实现商品排序（按价格、销量、评分等）
- [ ] 修改商品详情页面 (shop/intelligent-marble-knife/index.html)
  - [ ] 从后端API获取商品详情数据
  - [ ] 动态渲染商品信息（名称、价格、图片、描述等）
  - [ ] 实现"Add to cart"按钮功能
  - [ ] 实现数量选择器
  - [ ] 实现图片画廊切换
  - [ ] 从后端API获取相关商品（displayPosition=product_related，最多4个）
  - [ ] 动态渲染Related products区域的商品卡片
- [ ] 实现购物车功能 (cart/index.html)
  - [ ] 从后端API获取购物车数据
  - [ ] 实现商品数量修改
  - [ ] 实现商品删除功能
  - [ ] 实时计算总价
  - [ ] 更新顶部购物车徽章数字
- [ ] 实现心愿单功能 (wishlist/index.html)
  - [ ] 从后端API获取心愿单数据
  - [ ] 实现移除心愿单商品
  - [ ] 实现从心愿单添加到购物车
- [ ] 实现订单功能 (order-received/index.html)
  - [ ] 实现收货地址管理（新西兰格式）
  - [ ] 实现订单创建
  - [ ] 显示订单详情（含新西兰地址格式）
- [ ] 添加商品搜索功能（顶部搜索框）
- [ ] 添加商品筛选功能（分类、价格范围等）

**注意事项**：
- ⚠️ **前端不提供商品的添加/删除功能** - 商品的增删改只能在后台管理系统中进行
- ✅ **前端只提供商品的浏览和购买功能** - 包括查看商品、加入购物车、加入心愿单、下单等
- ✅ **购物车中的"删除"是指从购物车移除商品** - 不是删除商品本身
- ✅ **三个展示位置的商品由后台管理员配置** - 首页精选、商品列表、相关商品

### 阶段四：后台管理页面（2-3天）

**任务清单：**
- [ ] 创建商品管理菜单
- [ ] 商品列表页面（含搜索、筛选、分页）
- [ ] 商品新增页面（含图片上传、富文本编辑器、位置管理）
- [ ] 商品编辑页面（含图片上传、富文本编辑器、位置管理）
- [ ] 商品删除功能（含批量删除）
- [ ] 商品分类管理页面（树形结构）
- [ ] **商品位置管理页面** ⭐ 新增
  - [ ] 首页精选商品管理（最多4个）
  - [ ] 相关商品管理（最多4个）
  - [ ] 商品列表排序管理
  - [ ] 商品选择器（弹窗选择）
  - [ ] 拖拽排序功能
- [ ] 订单管理页面（含新西兰地址显示）
- [ ] 图片上传功能（支持多图上传）

### 阶段五：测试与优化（1-2天）

**任务清单：**
- [ ] 功能测试
- [ ] 性能优化
- [ ] 安全测试
- [ ] 用户体验优化

---

## 🎛️ 后台管理页面详细设计

### 1. 商品管理菜单结构

```
商品管理
├── 商品列表      /system/product/list
├── 新增商品      /system/product/add
├── 商品分类      /system/product/category
├── 商品位置管理  /system/product/position
└── 订单管理      /system/order/list
```

**商品位置管理说明**：
- **首页精选商品** (`home_featured`): 首页 Featured products 区域展示的4个商品
- **商品列表** (`shop_list`): 商品列表页面展示的所有商品
- **相关商品** (`product_related`): 商品详情页下方 Related products 区域展示的4个商品

### 2. 商品列表页面

**页面路径**: `/system/product/list`

**功能模块**:
- ✅ **搜索栏**: 商品名称、商品编码(SKU)搜索
- ✅ **筛选器**:
  - 商品分类（下拉树形选择）
  - 商品状态（上架/下架）
  - 是否精选/新品/热销
  - 创建时间范围
- ✅ **操作按钮**:
  - 新增商品（跳转到新增页面）
  - 批量删除（选中多个商品后删除）
  - 批量上架/下架
  - 导出Excel
- ✅ **表格列**:
  - 商品ID
  - 商品图片（缩略图）
  - 商品名称
  - SKU编码
  - 分类
  - 价格/原价
  - 库存
  - 销量
  - 状态（上架/下架）
  - 创建时间
  - 操作（编辑/删除/查看）
- ✅ **分页**: 每页10/20/50条可选

**页面示例代码结构**:
```vue
<template>
  <div class="product-list">
    <!-- 搜索栏 -->
    <el-form :inline="true" :model="queryParams">
      <el-form-item label="商品名称">
        <el-input v-model="queryParams.productName" placeholder="请输入商品名称" />
      </el-form-item>
      <el-form-item label="商品分类">
        <el-tree-select v-model="queryParams.categoryId" :data="categoryTree" />
      </el-form-item>
      <el-form-item label="商品状态">
        <el-select v-model="queryParams.status">
          <el-option label="全部" value="" />
          <el-option label="上架" value="0" />
          <el-option label="下架" value="1" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="handleQuery">搜索</el-button>
        <el-button @click="resetQuery">重置</el-button>
      </el-form-item>
    </el-form>

    <!-- 操作按钮 -->
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" icon="Plus" @click="handleAdd">新增商品</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" icon="Delete" @click="handleBatchDelete">批量删除</el-button>
      </el-col>
    </el-row>

    <!-- 商品表格 -->
    <el-table :data="productList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" />
      <el-table-column label="商品图片" width="100">
        <template #default="scope">
          <el-image :src="scope.row.mainImage" style="width: 60px; height: 60px" />
        </template>
      </el-table-column>
      <el-table-column label="商品名称" prop="productName" />
      <el-table-column label="SKU" prop="productCode" />
      <el-table-column label="分类" prop="categoryName" />
      <el-table-column label="价格" prop="price" />
      <el-table-column label="库存" prop="stock" />
      <el-table-column label="状态">
        <template #default="scope">
          <el-tag :type="scope.row.status === '0' ? 'success' : 'danger'">
            {{ scope.row.status === '0' ? '上架' : '下架' }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="200">
        <template #default="scope">
          <el-button type="primary" link @click="handleEdit(scope.row)">编辑</el-button>
          <el-button type="danger" link @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 分页 -->
    <pagination
      v-model:page="queryParams.pageNum"
      v-model:limit="queryParams.pageSize"
      :total="total"
      @pagination="getList"
    />
  </div>
</template>
```

### 3. 商品新增/编辑页面

**页面路径**: `/system/product/add` 或 `/system/product/edit/:id`

**表单字段**:

**基本信息**:
- 商品名称 *（必填）
- 商品编码(SKU) *（必填，唯一）
- 商品分类 *（必填，树形选择）
- 商品标签（多选）

**价格库存**:
- 售价 *（必填）
- 原价
- 成本价
- 库存数量 *（必填）
- 单位（默认"件"）
- 重量(kg)
- 体积(m³)

**商品图片**:
- 主图上传 *（必填，单张）
- 商品图片（多图上传，最多8张）

**商品描述**:
- 简短描述（500字以内）
- 详细描述 *（必填，富文本编辑器）

**商品属性**:
- 规格参数（JSON编辑器或动态表单）
- 是否精选
- 是否新品
- 是否热销

**其他**:
- 商品状态（上架/下架）

**展示位置** ⭐ 新增:
- 展示位置（多选）:
  - ☑ 首页精选商品 (home_featured) - 最多选4个商品
  - ☑ 商品列表 (shop_list) - 默认所有商品
  - ☑ 相关商品 (product_related) - 最多选4个商品
- 展示顺序（数字，越小越靠前）
- 排序值
- 备注

**页面示例代码结构**:
```vue
<template>
  <div class="product-form">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
      <!-- 基本信息 -->
      <el-divider content-position="left">基本信息</el-divider>
      <el-form-item label="商品名称" prop="productName">
        <el-input v-model="form.productName" placeholder="请输入商品名称" />
      </el-form-item>
      <el-form-item label="商品编码" prop="productCode">
        <el-input v-model="form.productCode" placeholder="请输入SKU编码" />
      </el-form-item>
      <el-form-item label="商品分类" prop="categoryId">
        <el-tree-select v-model="form.categoryId" :data="categoryTree" />
      </el-form-item>

      <!-- 价格库存 -->
      <el-divider content-position="left">价格库存</el-divider>
      <el-form-item label="售价" prop="price">
        <el-input-number v-model="form.price" :min="0" :precision="2" />
      </el-form-item>
      <el-form-item label="原价" prop="originalPrice">
        <el-input-number v-model="form.originalPrice" :min="0" :precision="2" />
      </el-form-item>
      <el-form-item label="库存" prop="stock">
        <el-input-number v-model="form.stock" :min="0" />
      </el-form-item>

      <!-- 商品图片 -->
      <el-divider content-position="left">商品图片</el-divider>
      <el-form-item label="主图" prop="mainImage">
        <el-upload
          class="avatar-uploader"
          :action="uploadUrl"
          :show-file-list="false"
          :on-success="handleMainImageSuccess"
        >
          <img v-if="form.mainImage" :src="form.mainImage" class="avatar" />
          <el-icon v-else class="avatar-uploader-icon"><Plus /></el-icon>
        </el-upload>
      </el-form-item>
      <el-form-item label="商品图片">
        <el-upload
          :action="uploadUrl"
          list-type="picture-card"
          :file-list="imageList"
          :on-success="handleImagesSuccess"
          :limit="8"
        >
          <el-icon><Plus /></el-icon>
        </el-upload>
      </el-form-item>

      <!-- 商品描述 -->
      <el-divider content-position="left">商品描述</el-divider>
      <el-form-item label="简短描述" prop="shortDescription">
        <el-input
          v-model="form.shortDescription"
          type="textarea"
          :rows="3"
          maxlength="500"
          show-word-limit
        />
      </el-form-item>
      <el-form-item label="详细描述" prop="description">
        <editor v-model="form.description" />
      </el-form-item>

      <!-- 商品属性 -->
      <el-divider content-position="left">商品属性</el-divider>
      <el-form-item label="是否精选">
        <el-switch v-model="form.isFeatured" :active-value="1" :inactive-value="0" />
      </el-form-item>
      <el-form-item label="是否新品">
        <el-switch v-model="form.isNew" :active-value="1" :inactive-value="0" />
      </el-form-item>
      <el-form-item label="是否热销">
        <el-switch v-model="form.isHot" :active-value="1" :inactive-value="0" />
      </el-form-item>
      <el-form-item label="商品状态">
        <el-radio-group v-model="form.status">
          <el-radio label="0">上架</el-radio>
          <el-radio label="1">下架</el-radio>
        </el-radio-group>
      </el-form-item>

      <!-- 提交按钮 -->
      <el-form-item>
        <el-button type="primary" @click="submitForm">保存</el-button>
        <el-button @click="cancel">取消</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
```

### 4. 商品位置管理页面 ⭐ 新增

**页面路径**: `/system/product/position`

**功能说明**：
管理商品在前端页面的展示位置，包括首页精选、商品列表、相关商品三个区域。

**页面布局**：

```vue
<template>
  <div class="product-position-management">
    <el-tabs v-model="activeTab" type="card">
      <!-- Tab 1: 首页精选商品 -->
      <el-tab-pane label="首页精选商品 (Featured Products)" name="home_featured">
        <el-alert
          title="提示：首页精选商品最多展示4个，请选择最优质的商品进行展示"
          type="info"
          :closable="false"
          style="margin-bottom: 20px"
        />

        <!-- 已选商品列表 -->
        <div class="selected-products">
          <h3>已选商品 ({{ featuredProducts.length }}/4)</h3>
          <el-table :data="featuredProducts" row-key="productId">
            <el-table-column label="排序" width="80">
              <template #default="scope">
                <el-input-number v-model="scope.row.displayOrder" :min="1" :max="100" size="small" />
              </template>
            </el-table-column>
            <el-table-column label="商品图片" width="100">
              <template #default="scope">
                <el-image :src="scope.row.mainImage" style="width: 60px; height: 60px" />
              </template>
            </el-table-column>
            <el-table-column label="商品名称" prop="productName" />
            <el-table-column label="SKU" prop="productCode" width="120" />
            <el-table-column label="价格" prop="price" width="100" />
            <el-table-column label="操作" width="100">
              <template #default="scope">
                <el-button type="danger" link @click="removeFromPosition(scope.row, 'home_featured')">
                  移除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>

        <!-- 添加商品按钮 -->
        <el-button
          type="primary"
          icon="Plus"
          @click="openProductSelector('home_featured')"
          :disabled="featuredProducts.length >= 4"
          style="margin-top: 20px"
        >
          添加商品到首页精选
        </el-button>
      </el-tab-pane>

      <!-- Tab 2: 相关商品 -->
      <el-tab-pane label="相关商品 (Related Products)" name="product_related">
        <el-alert
          title="提示：相关商品最多展示4个，会在商品详情页下方显示"
          type="info"
          :closable="false"
          style="margin-bottom: 20px"
        />

        <!-- 已选商品列表 -->
        <div class="selected-products">
          <h3>已选商品 ({{ relatedProducts.length }}/4)</h3>
          <el-table :data="relatedProducts" row-key="productId">
            <el-table-column label="排序" width="80">
              <template #default="scope">
                <el-input-number v-model="scope.row.displayOrder" :min="1" :max="100" size="small" />
              </template>
            </el-table-column>
            <el-table-column label="商品图片" width="100">
              <template #default="scope">
                <el-image :src="scope.row.mainImage" style="width: 60px; height: 60px" />
              </template>
            </el-table-column>
            <el-table-column label="商品名称" prop="productName" />
            <el-table-column label="SKU" prop="productCode" width="120" />
            <el-table-column label="价格" prop="price" width="100" />
            <el-table-column label="操作" width="100">
              <template #default="scope">
                <el-button type="danger" link @click="removeFromPosition(scope.row, 'product_related')">
                  移除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>

        <!-- 添加商品按钮 -->
        <el-button
          type="primary"
          icon="Plus"
          @click="openProductSelector('product_related')"
          :disabled="relatedProducts.length >= 4"
          style="margin-top: 20px"
        >
          添加商品到相关商品
        </el-button>
      </el-tab-pane>

      <!-- Tab 3: 商品列表 -->
      <el-tab-pane label="商品列表 (Shop List)" name="shop_list">
        <el-alert
          title="提示：商品列表展示所有上架的商品，可以调整展示顺序"
          type="info"
          :closable="false"
          style="margin-bottom: 20px"
        />

        <!-- 商品列表 -->
        <el-table :data="shopListProducts" row-key="productId">
          <el-table-column label="排序" width="80">
            <template #default="scope">
              <el-input-number v-model="scope.row.displayOrder" :min="1" :max="1000" size="small" />
            </template>
          </el-table-column>
          <el-table-column label="商品图片" width="100">
            <template #default="scope">
              <el-image :src="scope.row.mainImage" style="width: 60px; height: 60px" />
            </template>
          </el-table-column>
          <el-table-column label="商品名称" prop="productName" />
          <el-table-column label="SKU" prop="productCode" width="120" />
          <el-table-column label="分类" prop="categoryName" width="120" />
          <el-table-column label="价格" prop="price" width="100" />
          <el-table-column label="状态" width="80">
            <template #default="scope">
              <el-tag :type="scope.row.status === '0' ? 'success' : 'danger'">
                {{ scope.row.status === '0' ? '上架' : '下架' }}
              </el-tag>
            </template>
          </el-table-column>
        </el-table>

        <!-- 分页 -->
        <pagination
          v-model:page="queryParams.pageNum"
          v-model:limit="queryParams.pageSize"
          :total="total"
          @pagination="getShopListProducts"
        />
      </el-tab-pane>
    </el-tabs>

    <!-- 保存按钮 -->
    <el-row style="margin-top: 20px">
      <el-button type="primary" size="large" @click="savePositions">保存所有位置设置</el-button>
      <el-button size="large" @click="resetPositions">重置</el-button>
    </el-row>

    <!-- 商品选择器对话框 -->
    <el-dialog v-model="productSelectorVisible" title="选择商品" width="80%">
      <el-form :inline="true">
        <el-form-item label="商品名称">
          <el-input v-model="selectorQuery.productName" placeholder="请输入商品名称" />
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="searchProducts">搜索</el-button>
        </el-form-item>
      </el-form>

      <el-table :data="availableProducts" @selection-change="handleProductSelection">
        <el-table-column type="selection" width="55" />
        <el-table-column label="商品图片" width="100">
          <template #default="scope">
            <el-image :src="scope.row.mainImage" style="width: 60px; height: 60px" />
          </template>
        </el-table-column>
        <el-table-column label="商品名称" prop="productName" />
        <el-table-column label="SKU" prop="productCode" width="120" />
        <el-table-column label="价格" prop="price" width="100" />
      </el-table>

      <template #footer>
        <el-button @click="productSelectorVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmProductSelection">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { getProductsByPosition, updateProductPosition } from '@/api/system/product'

const activeTab = ref('home_featured')
const featuredProducts = ref([])
const relatedProducts = ref([])
const shopListProducts = ref([])
const productSelectorVisible = ref(false)
const currentPosition = ref('')
const selectedProducts = ref([])

// 加载各位置的商品
const loadPositionProducts = async () => {
  // 加载首页精选商品
  const featuredRes = await getProductsByPosition('home_featured')
  featuredProducts.value = featuredRes.data

  // 加载相关商品
  const relatedRes = await getProductsByPosition('product_related')
  relatedProducts.value = relatedRes.data

  // 加载商品列表
  const shopListRes = await getProductsByPosition('shop_list')
  shopListProducts.value = shopListRes.data
}

// 打开商品选择器
const openProductSelector = (position) => {
  currentPosition.value = position
  productSelectorVisible.value = true
  searchProducts()
}

// 从位置移除商品
const removeFromPosition = async (product, position) => {
  // 调用API移除商品位置
  await updateProductPosition(product.productId, { position, action: 'remove' })
  loadPositionProducts()
}

// 保存所有位置设置
const savePositions = async () => {
  // 保存所有商品的位置和顺序
  const positions = [
    ...featuredProducts.value.map(p => ({ ...p, position: 'home_featured' })),
    ...relatedProducts.value.map(p => ({ ...p, position: 'product_related' })),
    ...shopListProducts.value.map(p => ({ ...p, position: 'shop_list' }))
  ]

  await updateProductPosition(null, { positions })
  ElMessage.success('保存成功')
}

onMounted(() => {
  loadPositionProducts()
})
</script>
```

**功能特点**：
1. ✅ **分标签页管理**：首页精选、相关商品、商品列表三个独立标签页
2. ✅ **数量限制**：首页精选和相关商品最多4个，超过后禁用添加按钮
3. ✅ **拖拽排序**：支持通过输入数字调整展示顺序
4. ✅ **商品选择器**：弹窗选择商品，支持搜索和多选
5. ✅ **实时预览**：显示已选商品的图片、名称、价格等信息
6. ✅ **批量保存**：一次性保存所有位置的设置

---

### 5. 商品分类管理页面

**页面路径**: `/system/product/category`

**功能模块**:
- ✅ 树形表格展示分类层级
- ✅ 新增分类（支持多级分类）
- ✅ 编辑分类
- ✅ 删除分类（检查是否有商品使用）
- ✅ 拖拽排序

### 6. 订单管理页面

**页面路径**: `/system/order/list`

**功能模块**:
- ✅ **订单列表**（含搜索、筛选）
  - 订单号搜索
  - 订单状态筛选（待支付、待发货、已发货、已完成、已取消）
  - 下单时间范围筛选
  - 客户姓名/电话搜索
- ✅ **订单详情查看**
  - 订单基本信息（订单号、下单时间、订单状态、支付方式）
  - 商品清单（商品名称、SKU、数量、单价、小计）
  - 收货地址（新西兰地址格式）
  - 订单金额（商品总额、运费、优惠、实付金额）
- ✅ **订单状态修改**
  - 发货（输入物流单号）
  - 完成订单
  - 取消订单
- ✅ **订单导出**（Excel格式）

**收货地址显示格式**（新西兰）:
```
收货人：John Smith
联系电话：021 123 4567
收货地址：
  123 Queen Street
  Ponsonby
  Auckland, Auckland Region
  1011
  New Zealand
配送说明：Leave at front door
```

**订单详情页面示例**:
```vue
<template>
  <el-dialog v-model="visible" title="订单详情" width="80%">
    <el-descriptions :column="2" border>
      <el-descriptions-item label="订单号">{{ order.orderNo }}</el-descriptions-item>
      <el-descriptions-item label="下单时间">{{ order.createTime }}</el-descriptions-item>
      <el-descriptions-item label="订单状态">
        <el-tag :type="getStatusType(order.status)">{{ getStatusText(order.status) }}</el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="支付方式">{{ order.paymentMethod }}</el-descriptions-item>
    </el-descriptions>

    <el-divider content-position="left">收货地址</el-divider>
    <el-descriptions :column="1" border>
      <el-descriptions-item label="收货人">{{ order.shippingAddress.receiverName }}</el-descriptions-item>
      <el-descriptions-item label="联系电话">{{ order.shippingAddress.receiverPhone }}</el-descriptions-item>
      <el-descriptions-item label="收货地址">
        <div>{{ order.shippingAddress.streetAddress }}</div>
        <div v-if="order.shippingAddress.suburb">{{ order.shippingAddress.suburb }}</div>
        <div>{{ order.shippingAddress.city }}, {{ order.shippingAddress.region }}</div>
        <div>{{ order.shippingAddress.postalCode }}</div>
        <div>{{ order.shippingAddress.country }}</div>
      </el-descriptions-item>
      <el-descriptions-item label="配送说明" v-if="order.shippingAddress.deliveryInstructions">
        {{ order.shippingAddress.deliveryInstructions }}
      </el-descriptions-item>
    </el-descriptions>

    <el-divider content-position="left">商品清单</el-divider>
    <el-table :data="order.items" border>
      <el-table-column label="商品图片" width="100">
        <template #default="scope">
          <el-image :src="scope.row.productImage" style="width: 60px; height: 60px" />
        </template>
      </el-table-column>
      <el-table-column label="商品名称" prop="productName" />
      <el-table-column label="SKU" prop="productCode" width="120" />
      <el-table-column label="单价" prop="price" width="100" />
      <el-table-column label="数量" prop="quantity" width="80" />
      <el-table-column label="小计" width="100">
        <template #default="scope">
          ${{ (scope.row.price * scope.row.quantity).toFixed(2) }}
        </template>
      </el-table-column>
    </el-table>

    <el-divider content-position="left">订单金额</el-divider>
    <el-descriptions :column="1" border>
      <el-descriptions-item label="商品总额">${{ order.totalAmount }}</el-descriptions-item>
      <el-descriptions-item label="运费">${{ order.shippingFee }}</el-descriptions-item>
      <el-descriptions-item label="优惠金额">-${{ order.discountAmount }}</el-descriptions-item>
      <el-descriptions-item label="实付金额">
        <span style="color: #f56c6c; font-size: 18px; font-weight: bold">
          ${{ order.paymentAmount }}
        </span>
      </el-descriptions-item>
    </el-descriptions>

    <template #footer>
      <el-button @click="visible = false">关闭</el-button>
      <el-button type="primary" @click="handleShip" v-if="order.status === 'pending_ship'">
        发货
      </el-button>
      <el-button type="success" @click="handleComplete" v-if="order.status === 'shipped'">
        完成订单
      </el-button>
      <el-button type="danger" @click="handleCancel" v-if="order.status === 'pending_payment'">
        取消订单
      </el-button>
    </template>
  </el-dialog>
</template>
```

## � 技术要点和最佳实践

### 1. 商品位置管理实现

**数据库设计**：
- `displayPosition` 字段存储多个位置（逗号分隔）：`"home_featured,shop_list"`
- `displayOrder` 字段控制同一位置内的排序（数字越小越靠前）

**前端API查询示例**：
```typescript
// 获取首页精选商品（最多4个）
GET /api/frontend/product/list?displayPosition=home_featured&pageSize=4&sortBy=displayOrder&sortOrder=asc

// 获取商品列表页商品（所有上架商品）
GET /api/frontend/product/list?displayPosition=shop_list&pageNum=1&pageSize=12&sortBy=displayOrder&sortOrder=asc

// 获取相关商品（最多4个）
GET /api/frontend/product/list?displayPosition=product_related&pageSize=4&sortBy=displayOrder&sortOrder=asc
```

**Prisma查询示例**：
```typescript
// 查询包含home_featured位置的商品
const products = await prisma.product.findMany({
  where: {
    displayPosition: {
      contains: 'home_featured'
    },
    status: '0',
    delFlag: '0'
  },
  orderBy: {
    displayOrder: 'asc'
  },
  take: 4
});
```

### 2. 新西兰地址格式处理

**前端显示格式**：
```
John Smith
021 123 4567
123 Queen Street
Ponsonby
Auckland, Auckland Region
1011
New Zealand
```

**数据库存储**：
- 使用独立的 `customer_address` 表存储
- 订单表的 `shippingAddress` 字段存储JSON格式的地址快照

**地址验证规则**：
- 邮政编码：4位数字（1000-9999）
- 电话号码：支持固定电话（如 09 123 4567）和手机（如 021 123 4567）
- 主要城市：Auckland, Wellington, Christchurch, Hamilton, Tauranga等

### 3. 图片上传和存储

**上传路径配置**：
- 配置在 `backend/meimei-prisma-vue3/meimei-admin/src/config/config.dev.ts` 的 `uploadPath` 字段
- 默认路径：项目根目录下 `/static/upload/`
- 建议路径：`/static/upload/product/{年月}/{随机文件名}.jpg`

**图片字段存储**：
- `mainImage`: 主图（单张），存储相对路径字符串
- `images`: 多图（JSON数组字符串）
  ```json
  ["uploads/product/202411/abc123.jpg", "uploads/product/202411/def456.jpg"]
  ```

**前端访问URL**：
- 完整URL：`http://localhost:3000/uploads/product/202411/abc123.jpg`
- 静态资源由NestJS的静态文件服务提供

**图片尺寸要求和响应式处理** ⭐ 重要：

根据前端页面实际使用的响应式断点和图片尺寸分析：

**前端响应式断点**：
- **Desktop（桌面）**: > 1366px
- **Laptop（笔记本）**: ≤ 1366px
- **Tablet Extra（大平板）**: ≤ 1200px
- **Tablet（平板）**: ≤ 1024px
- **Mobile Extra（大手机）**: ≤ 880px
- **Mobile（手机）**: ≤ 767px

**前端实际使用的图片尺寸**（参考 `frontend/wp-content/uploads/2021/11/` 目录）：

WordPress自动生成的尺寸：
- **原图**: 800x877px（或更大）
- **768x842**: 用于大屏幕（保持比例）
- **600x600**: 正方形中图
- **460x460**: 正方形中图
- **450x450**: 正方形中图
- **274x300**: 小图（保持比例）
- **150x150**: 缩略图（正方形）
- **100x100**: 超小缩略图（正方形）
- **96x96**: 超小缩略图（正方形）
- **50x50**: 微型缩略图（正方形）

**四个使用商品图片的位置**：

1. **首页（index.html）- Featured Products区域**：
   - 原图：800x877px
   - srcset：`800w, 274x300, 768x842`
   - sizes：`(max-width: 800px) 100vw, 800px`
   - 布局：4列（columns-1），每列100%宽度
   - 实际显示：约400-600px

2. **商品列表页（shop/index.html）**：
   - 原图：800x877px
   - srcset：`800w, 274x300, 768x842`
   - sizes：`(max-width: 800px) 100vw, 800px`
   - 布局：响应式1-4列
   - 实际显示：Desktop约300-400px，Tablet约350-500px，Mobile约300-400px

3. **商品详情页（intelligent-marble-knife/index.html）- 主图区域**：
   - 主图：800x877px
   - srcset：`800w, 274x300, 768x842`
   - sizes：`(max-width: 800px) 100vw, 800px`
   - 实际显示：Desktop约500-600px，Tablet约400-500px，Mobile约300-400px

4. **商品详情页 - 左侧缩略图列表** ⭐ 新增：
   - data-thumb-sizes：`(max-width: 109px) 100vw, 109px`
   - 实际显示：约100-109px
   - 用途：点击切换主图

5. **心愿单/比较功能**：
   - 使用：150x150px
   - 用途：心愿单、比较列表的商品缩略图

**推荐生成的图片尺寸**（完全匹配前端实际需求）：
- **原图（original）**: 保留上传的原图（最大1200px宽度，保持比例）
- **大图（large_800）**: **800x877px** - 保持原比例，用于商品详情页主图、列表页
- **大图（large_768）**: **768x842px** - 保持原比例，用于大屏幕
- **中图（medium_600）**: **600x600px** - 正方形裁剪，用于桌面端商品列表、首页
- **中图（medium_460）**: **460x460px** - 正方形裁剪，备用尺寸
- **小图（small_274）**: **274x300px** - 保持比例，用于移动端
- **缩略图（thumbnail_150）**: **150x150px** - 正方形裁剪，用于心愿单、购物车
- **微缩图（thumbnail_100）**: **100x100px** - 正方形裁剪，用于详情页左侧缩略图列表
- **WebP格式**: 同时生成600x600和768x842的WebP格式（体积更小30-50%）

- **响应式图片实现方案**：

  **方案一：前端CSS响应式（推荐）**
  ```css
  /* 商品图片容器 */
  .product-image {
    width: 100%;
    height: auto;
    object-fit: cover; /* 保持比例裁剪 */
    aspect-ratio: 1 / 1; /* 保持正方形 */
  }

  /* 商品列表卡片图片 */
  .product-card img {
    width: 100%;
    height: auto;
    max-width: 400px;
    object-fit: cover;
  }

  /* 商品详情页主图 */
  .product-detail-image {
    width: 100%;
    max-width: 600px;
    height: auto;
  }

  /* 移动端优化 */
  @media (max-width: 768px) {
    .product-image {
      max-width: 100%;
    }
  }
  ```

  **方案二：后端自动生成多尺寸图片（推荐，已采用）** ⭐
  - 使用 `sharp` 库在上传时自动生成多个尺寸
  - 存储格式（JSON字符串存储到数据库的 `mainImage` 和 `images` 字段）：
    ```json
    {
      "original": "uploads/product/202411/abc123.jpg",
      "large_800": "uploads/product/202411/abc123_800x877.jpg",
      "large_768": "uploads/product/202411/abc123_768x842.jpg",
      "medium_600": "uploads/product/202411/abc123_600x600.jpg",
      "medium_460": "uploads/product/202411/abc123_460x460.jpg",
      "small_274": "uploads/product/202411/abc123_274x300.jpg",
      "thumbnail_150": "uploads/product/202411/abc123_150x150.jpg",
      "thumbnail_100": "uploads/product/202411/abc123_100x100.jpg",
      "webp_600": "uploads/product/202411/abc123_600x600.webp",
      "webp_768": "uploads/product/202411/abc123_768x842.webp"
    }
    ```

  - **后端图片处理服务完整代码**：
    ```typescript
    // backend/meimei-prisma-vue3/meimei-admin/src/modules/product/image-processor.service.ts
    import { Injectable } from '@nestjs/common';
    import * as sharp from 'sharp';
    import * as path from 'path';
    import * as fs from 'fs';
    import { v4 as uuidv4 } from 'uuid';

    @Injectable()
    export class ImageProcessorService {
      private uploadPath = './static/upload/product';

      /**
       * 处理商品图片上传，自动生成多个尺寸
       * @param file 上传的文件
       * @returns 返回各尺寸图片的路径对象
       */
      async processProductImage(file: Express.Multer.File): Promise<any> {
        // 创建年月目录
        const now = new Date();
        const yearMonth = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}`;
        const uploadDir = path.join(this.uploadPath, yearMonth);

        // 确保目录存在
        if (!fs.existsSync(uploadDir)) {
          fs.mkdirSync(uploadDir, { recursive: true });
        }

        // 生成唯一文件名
        const fileName = uuidv4();
        const ext = '.jpg'; // 统一转换为jpg格式

        // 定义各尺寸（完全匹配前端实际需求）
        const sizes = {
          original: { width: null, height: null, suffix: '', quality: 90, fit: 'inside' },           // 保留原图（最大1200px）
          large_800: { width: 800, height: 877, suffix: '_800x877', quality: 85, fit: 'inside' },    // 商品详情页主图、列表页（保持比例）
          large_768: { width: 768, height: 842, suffix: '_768x842', quality: 85, fit: 'inside' },    // 大屏幕（保持比例）
          medium_600: { width: 600, height: 600, suffix: '_600x600', quality: 85, fit: 'cover' },    // 桌面端商品列表、首页（正方形）
          medium_460: { width: 460, height: 460, suffix: '_460x460', quality: 85, fit: 'cover' },    // 备用尺寸（正方形）
          small_274: { width: 274, height: 300, suffix: '_274x300', quality: 85, fit: 'inside' },    // 移动端（保持比例）
          thumbnail_150: { width: 150, height: 150, suffix: '_150x150', quality: 80, fit: 'cover' }, // 心愿单、购物车（正方形）
          thumbnail_100: { width: 100, height: 100, suffix: '_100x100', quality: 80, fit: 'cover' }  // 详情页左侧缩略图（正方形）
        };

        const result = {};

        // 生成各尺寸图片
        for (const [key, config] of Object.entries(sizes)) {
          const outputFileName = `${fileName}${config.suffix}${ext}`;
          const outputPath = path.join(uploadDir, outputFileName);
          const relativePath = `uploads/product/${yearMonth}/${outputFileName}`;

          let sharpInstance = sharp(file.buffer);

          if (config.width && config.height) {
            // 调整尺寸
            sharpInstance = sharpInstance.resize(config.width, config.height, {
              fit: config.fit,        // 'cover'=裁剪为正方形, 'inside'=保持比例
              position: 'center',     // 居中裁剪/对齐
              withoutEnlargement: true
            });
          } else {
            // 原图：限制最大宽度为1200px
            const metadata = await sharp(file.buffer).metadata();
            if (metadata.width > 1200) {
              sharpInstance = sharpInstance.resize(1200, null, {
                fit: 'inside',        // 保持比例，不裁剪
                withoutEnlargement: true
              });
            }
          }

          // 转换为jpg格式，压缩
          await sharpInstance
            .jpeg({ quality: config.quality })
            .toFile(outputPath);

          result[key] = relativePath;
        }

        // 同时生成WebP格式（用于现代浏览器，体积更小30-50%）
        // 生成600x600正方形WebP
        const webp600FileName = `${fileName}_600x600.webp`;
        const webp600Path = path.join(uploadDir, webp600FileName);
        await sharp(file.buffer)
          .resize(600, 600, { fit: 'cover', position: 'center' })
          .webp({ quality: 80 })
          .toFile(webp600Path);
        result['webp_600'] = `uploads/product/${yearMonth}/${webp600FileName}`;

        // 生成768x842保持比例WebP
        const webp768FileName = `${fileName}_768x842.webp`;
        const webp768Path = path.join(uploadDir, webp768FileName);
        await sharp(file.buffer)
          .resize(768, 842, { fit: 'inside', position: 'center' })
          .webp({ quality: 80 })
          .toFile(webp768Path);
        result['webp_768'] = `uploads/product/${yearMonth}/${webp768FileName}`;

        return result;
      }

      /**
       * 删除商品图片（删除所有尺寸）
       * @param imagePath 图片路径对象或主图路径字符串
       */
      async deleteProductImage(imagePath: any): Promise<void> {
        if (typeof imagePath === 'string') {
          // 单个路径字符串
          const fullPath = path.join('./', imagePath);
          if (fs.existsSync(fullPath)) {
            fs.unlinkSync(fullPath);
          }
        } else if (typeof imagePath === 'object' && imagePath !== null) {
          // 多尺寸路径对象
          for (const imgPath of Object.values(imagePath)) {
            if (typeof imgPath === 'string') {
              const fullPath = path.join('./', imgPath);
              if (fs.existsSync(fullPath)) {
                fs.unlinkSync(fullPath);
              }
            }
          }
        }
      }

      /**
       * 批量删除商品图片（用于删除商品时）
       * @param mainImage 主图路径对象
       * @param images 多图数组（每个元素是路径对象）
       */
      async deleteProductImages(mainImage: any, images: any[]): Promise<void> {
        // 删除主图
        if (mainImage) {
          await this.deleteProductImage(mainImage);
        }

        // 删除多图
        if (Array.isArray(images)) {
          for (const image of images) {
            await this.deleteProductImage(image);
          }
        }
      }
    }
    ```

  - 前端使用 `<picture>` 标签或 `srcset` 属性：
    ```html
    <!-- 使用srcset实现响应式图片 -->
    <img
      src="http://localhost:3000/uploads/product/202411/abc123_600x600.jpg"
      srcset="
        http://localhost:3000/uploads/product/202411/abc123_300x300.jpg 300w,
        http://localhost:3000/uploads/product/202411/abc123_600x600.jpg 600w,
        http://localhost:3000/uploads/product/202411/abc123_1200x1200.jpg 1200w
      "
      sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 400px"
      alt="Product Image"
      loading="lazy"
      class="product-image"
    />

    <!-- 使用picture标签支持WebP格式 -->
    <picture>
      <source
        type="image/webp"
        srcset="http://localhost:3000/uploads/product/202411/abc123_600x600.webp"
      />
      <img
        src="http://localhost:3000/uploads/product/202411/abc123_600x600.jpg"
        alt="Product Image"
        loading="lazy"
        class="product-image"
      />
    </picture>
    ```

  **方案三：使用CDN图片处理服务（生产环境推荐）**
  - 使用阿里云OSS、腾讯云COS等CDN服务的图片处理功能
  - URL参数控制尺寸：`https://cdn.example.com/product/abc123.jpg?x-oss-process=image/resize,w_600`
  - 优点：无需后端处理，CDN自动缓存，加载速度快

- **商品Service中的图片处理逻辑**：

  ```typescript
  // backend/meimei-prisma-vue3/meimei-admin/src/modules/product/product.service.ts
  import { Injectable } from '@nestjs/common';
  import { PrismaService } from '../prisma/prisma.service';
  import { ImageProcessorService } from './image-processor.service';

  @Injectable()
  export class ProductService {
    constructor(
      private prisma: PrismaService,
      private imageProcessor: ImageProcessorService
    ) {}

    /**
     * 创建商品（含图片上传）
     */
    async createProduct(data: any, mainImageFile?: Express.Multer.File, imageFiles?: Express.Multer.File[]) {
      // 处理主图
      let mainImagePaths = null;
      if (mainImageFile) {
        mainImagePaths = await this.imageProcessor.processProductImage(mainImageFile);
      }

      // 处理多图
      let imagePaths = [];
      if (imageFiles && imageFiles.length > 0) {
        for (const file of imageFiles) {
          const paths = await this.imageProcessor.processProductImage(file);
          imagePaths.push(paths);
        }
      }

      // 创建商品记录
      return await this.prisma.product.create({
        data: {
          ...data,
          mainImage: mainImagePaths ? JSON.stringify(mainImagePaths) : null,  // 存储为JSON字符串
          images: imagePaths.length > 0 ? JSON.stringify(imagePaths) : null,  // 存储为JSON字符串
          createTime: new Date(),
          updateTime: new Date()
        }
      });
    }

    /**
     * 修改商品（含图片更新）
     */
    async updateProduct(
      productId: number,
      data: any,
      mainImageFile?: Express.Multer.File,
      imageFiles?: Express.Multer.File[],
      deleteMainImage?: boolean,  // 是否删除主图
      deleteImageIndexes?: number[]  // 要删除的多图索引
    ) {
      // 获取原商品数据
      const product = await this.prisma.product.findUnique({
        where: { productId }
      });

      if (!product) {
        throw new Error('商品不存在');
      }

      // 解析原图片数据
      let oldMainImage = product.mainImage ? JSON.parse(product.mainImage) : null;
      let oldImages = product.images ? JSON.parse(product.images) : [];

      // 处理主图更新
      let newMainImage = oldMainImage;
      if (deleteMainImage && oldMainImage) {
        // 删除旧主图
        await this.imageProcessor.deleteProductImage(oldMainImage);
        newMainImage = null;
      }
      if (mainImageFile) {
        // 删除旧主图（如果存在）
        if (oldMainImage) {
          await this.imageProcessor.deleteProductImage(oldMainImage);
        }
        // 上传新主图
        newMainImage = await this.imageProcessor.processProductImage(mainImageFile);
      }

      // 处理多图更新
      let newImages = [...oldImages];

      // 删除指定的多图
      if (deleteImageIndexes && deleteImageIndexes.length > 0) {
        for (const index of deleteImageIndexes.sort((a, b) => b - a)) {  // 从后往前删除
          if (index >= 0 && index < newImages.length) {
            await this.imageProcessor.deleteProductImage(newImages[index]);
            newImages.splice(index, 1);
          }
        }
      }

      // 添加新的多图
      if (imageFiles && imageFiles.length > 0) {
        for (const file of imageFiles) {
          const paths = await this.imageProcessor.processProductImage(file);
          newImages.push(paths);
        }
      }

      // 更新商品记录
      return await this.prisma.product.update({
        where: { productId },
        data: {
          ...data,
          mainImage: newMainImage ? JSON.stringify(newMainImage) : null,
          images: newImages.length > 0 ? JSON.stringify(newImages) : null,
          updateTime: new Date()
        }
      });
    }

    /**
     * 删除商品（含图片删除）
     */
    async deleteProduct(productId: number) {
      // 获取商品数据
      const product = await this.prisma.product.findUnique({
        where: { productId }
      });

      if (!product) {
        throw new Error('商品不存在');
      }

      // 解析图片数据
      const mainImage = product.mainImage ? JSON.parse(product.mainImage) : null;
      const images = product.images ? JSON.parse(product.images) : [];

      // 删除所有图片文件
      await this.imageProcessor.deleteProductImages(mainImage, images);

      // 软删除商品记录
      return await this.prisma.product.update({
        where: { productId },
        data: {
          delFlag: '1',
          updateTime: new Date()
        }
      });
    }
  }
  ```

- **图片优化建议**：
  - ✅ 使用WebP格式（体积小30-50%，质量高）
  - ✅ 启用懒加载（`loading="lazy"`）
  - ✅ 添加图片占位符（避免布局抖动）
  - ✅ 压缩图片质量（80-85%即可，肉眼几乎无差别）
  - ✅ 使用CDN加速（生产环境）
  - ✅ 自动生成多尺寸（根据断点优化加载）

### 4. 购物车和订单流程

**购物车流程**：
1. 用户点击"Add to cart"按钮
2. 前端检查用户登录状态（JWT token）
3. 调用 `POST /api/frontend/product/cart/add` 接口
4. 后端检查商品库存是否充足
5. 添加到购物车表或更新数量
6. 返回成功，前端更新购物车徽章数字

**订单创建流程**：
1. 用户在购物车页面点击"Checkout"
2. 跳转到订单确认页面
3. 选择或新增收货地址（新西兰格式）
4. 调用 `POST /api/frontend/product/order/create` 接口
5. 后端创建订单记录和订单明细
6. 扣减商品库存
7. 清空购物车中已下单的商品
8. 返回订单号，跳转到订单详情页

### 5. 性能优化建议

**数据库优化**：
- ✅ 所有外键字段都已添加索引
- ✅ 常用查询字段（status, displayPosition）已添加索引
- ✅ 使用 `delFlag` 软删除，避免物理删除数据
- 建议：定期清理软删除的数据（delFlag='1'）

**缓存策略（可选）**：
- 商品列表：Redis缓存5分钟
- 商品详情：Redis缓存10分钟
- 商品分类树：Redis缓存30分钟
- 缓存key格式：`product:list:{categoryId}:{pageNum}:{pageSize}`

**前端优化**：
- 图片懒加载（Lazy Loading）
- 商品列表分页加载
- 使用CDN加速静态资源

### 6. 认证和授权

**前端用户认证**：
- 使用 `@UseGuards(CustomerJwtAuthGuard)` 装饰器
- Token存储在Redis：`customer_token:{customerId}`
- Token有效期：7天
- Token payload：`{ customerId, email, type: 'customer' }`

**后台管理员认证**：
- 使用 `@UseGuards(JwtAuthGuard)` 装饰器
- Token存储在Redis：`USER_TOKEN_KEY:{userId}`
- Token有效期：1年（由Redis管控实际过期时间）
- Token payload：`{ userId, pv }`（pv为密码版本号）

**公开接口**（无需认证）：
- 商品列表：`GET /api/frontend/product/list`
- 商品详情：`GET /api/frontend/product/:productId`
- 商品分类树：`GET /api/frontend/product/category/tree`

## 🔒 安全考虑

### 1. API鉴权
- **前端商品API**：购物车、订单、心愿单、收货地址需要JWT token验证
- **后台管理API**：所有接口需要管理员token验证
- **公开接口**：商品列表、商品详情、分类树等不需要登录

### 2. 数据验证
- 使用 `class-validator` 进行DTO验证
- 示例：
  ```typescript
  export class CreateProductDto {
    @IsNotEmpty({ message: '商品名称不能为空' })
    @MaxLength(200, { message: '商品名称最多200个字符' })
    productName: string;

    @IsNotEmpty({ message: '商品价格不能为空' })
    @IsNumber({}, { message: '商品价格必须是数字' })
    @Min(0.01, { message: '商品价格必须大于0' })
    price: number;

    @IsInt({ message: '库存必须是整数' })
    @Min(0, { message: '库存不能为负数' })
    stock: number;
  }
  ```

### 3. 权限控制和数据隔离 ⭐ 重要

**核心原则**：每个用户只能访问和操作自己的数据

#### **前端用户数据隔离**：

**1. 购物车数据隔离**：
```typescript
// 在Service中，customerId从JWT token中获取
async getCart(customerId: number) {
  // 只能查询当前用户的购物车
  return await this.prisma.shoppingCart.findMany({
    where: {
      customerId: customerId  // 从JWT token中获取，不能从请求参数获取
    },
    include: {
      product: true  // 关联查询商品信息
    }
  });
}

// 添加到购物车时也要验证
async addToCart(customerId: number, productId: number, quantity: number) {
  // customerId必须从JWT token中获取，不能信任前端传递的值
  return await this.prisma.shoppingCart.create({
    data: {
      customerId: customerId,  // 从token获取
      productId: productId,
      quantity: quantity
    }
  });
}
```

**2. 收货地址数据隔离**：
```typescript
// 获取地址列表
async getAddressList(customerId: number) {
  return await this.prisma.customerAddress.findMany({
    where: {
      customerId: customerId,  // 从JWT token中获取
      delFlag: '0'
    },
    orderBy: {
      isDefault: 'desc'  // 默认地址排在前面
    }
  });
}

// 添加/修改地址时验证所有权
async updateAddress(customerId: number, addressId: number, data: any) {
  // 先检查地址是否属于当前用户
  const address = await this.prisma.customerAddress.findFirst({
    where: {
      addressId: addressId,
      customerId: customerId  // 关键：确保地址属于当前用户
    }
  });

  if (!address) {
    throw new Error('地址不存在或无权限操作');
  }

  // 执行更新
  return await this.prisma.customerAddress.update({
    where: { addressId },
    data: data
  });
}

// 删除地址时验证所有权
async deleteAddress(customerId: number, addressId: number) {
  // 软删除，并验证所有权
  const result = await this.prisma.customerAddress.updateMany({
    where: {
      addressId: addressId,
      customerId: customerId  // 关键：只能删除自己的地址
    },
    data: {
      delFlag: '1'
    }
  });

  if (result.count === 0) {
    throw new Error('地址不存在或无权限操作');
  }

  return result;
}
```

**3. 订单数据隔离**：
```typescript
// 获取订单列表
async getOrderList(customerId: number, pageNum: number, pageSize: number) {
  return await this.prisma.customerOrder.findMany({
    where: {
      customerId: customerId,  // 从JWT token中获取
    },
    include: {
      orderItems: {
        include: {
          product: true
        }
      }
    },
    skip: (pageNum - 1) * pageSize,
    take: pageSize,
    orderBy: {
      createTime: 'desc'
    }
  });
}

// 获取订单详情
async getOrderDetail(customerId: number, orderId: number) {
  const order = await this.prisma.customerOrder.findFirst({
    where: {
      orderId: orderId,
      customerId: customerId  // 关键：确保订单属于当前用户
    },
    include: {
      orderItems: {
        include: {
          product: true
        }
      }
    }
  });

  if (!order) {
    throw new Error('订单不存在或无权限查看');
  }

  return order;
}
```

**4. 心愿单数据隔离**：
```typescript
// 获取心愿单
async getWishlist(customerId: number) {
  return await this.prisma.wishlist.findMany({
    where: {
      customerId: customerId  // 从JWT token中获取
    },
    include: {
      product: true
    }
  });
}

// 删除心愿单商品
async removeFromWishlist(customerId: number, wishlistId: number) {
  const result = await this.prisma.wishlist.deleteMany({
    where: {
      wishlistId: wishlistId,
      customerId: customerId  // 关键：只能删除自己的心愿单
    }
  });

  if (result.count === 0) {
    throw new Error('心愿单项不存在或无权限操作');
  }

  return result;
}
```

#### **Controller层获取customerId的标准方式**：

```typescript
import { Controller, Get, Post, Body, UseGuards, Request } from '@nestjs/common';
import { CustomerJwtAuthGuard } from '../frontend-auth/guards/customer-jwt-auth.guard';

@Controller('api/frontend/product')
export class ProductController {

  // 获取购物车
  @Get('cart/list')
  @UseGuards(CustomerJwtAuthGuard)  // 使用JWT守卫
  async getCart(@Request() req) {
    const customerId = req.user.customerId;  // 从JWT token中获取
    return await this.productService.getCart(customerId);
  }

  // 添加到购物车
  @Post('cart/add')
  @UseGuards(CustomerJwtAuthGuard)
  async addToCart(@Request() req, @Body() body: AddToCartDto) {
    const customerId = req.user.customerId;  // 从JWT token中获取，不从body获取
    return await this.productService.addToCart(
      customerId,
      body.productId,
      body.quantity
    );
  }

  // 获取收货地址列表
  @Get('address/list')
  @UseGuards(CustomerJwtAuthGuard)
  async getAddressList(@Request() req) {
    const customerId = req.user.customerId;  // 从JWT token中获取
    return await this.addressService.getAddressList(customerId);
  }

  // 创建订单
  @Post('order/create')
  @UseGuards(CustomerJwtAuthGuard)
  async createOrder(@Request() req, @Body() body: CreateOrderDto) {
    const customerId = req.user.customerId;  // 从JWT token中获取
    return await this.orderService.createOrder(customerId, body);
  }
}
```

#### **关键安全要点**：

1. ✅ **永远不要信任前端传递的customerId** - 必须从JWT token中获取
2. ✅ **所有查询都要加上customerId条件** - 确保数据隔离
3. ✅ **修改/删除操作前先验证所有权** - 使用 `findFirst` 或 `updateMany` 带customerId条件
4. ✅ **使用 `updateMany` 或 `deleteMany`** - 自动验证所有权，如果count=0说明无权限
5. ✅ **管理员查看所有订单时不加customerId限制** - 但要验证管理员权限

#### **管理员权限控制**：

```typescript
// 管理员查看所有订单（不限制customerId）
@Get('system/order/list')
@UseGuards(JwtAuthGuard)  // 管理员守卫
async getOrderListAdmin(@Request() req, @Query() query: OrderQueryDto) {
  // 管理员可以查看所有订单
  return await this.orderService.getOrderListAdmin(query);
}

// 管理员修改订单状态
@Put('system/order/:orderId/status')
@UseGuards(JwtAuthGuard)
async updateOrderStatus(
  @Param('orderId') orderId: number,
  @Body() body: UpdateOrderStatusDto
) {
  // 管理员可以修改任何订单
  return await this.orderService.updateOrderStatus(orderId, body.status);
}
```

### 4. SQL注入防护
- 使用Prisma ORM自动防护
- 避免使用原始SQL查询（`$queryRaw`）
- 如必须使用原始SQL，使用参数化查询

### 5. XSS防护
- 对用户输入进行转义处理
- 富文本内容使用DOMPurify清理
- 前端显示时使用 `textContent` 而非 `innerHTML`（除非是可信的富文本内容）

### 6. 库存并发控制
- 使用数据库事务确保库存扣减的原子性
- 示例：
  ```typescript
  await prisma.$transaction(async (tx) => {
    // 1. 检查库存
    const product = await tx.product.findUnique({
      where: { productId }
    });

    if (product.stock < quantity) {
      throw new Error('库存不足');
    }

    // 2. 扣减库存
    await tx.product.update({
      where: { productId },
      data: {
        stock: {
          decrement: quantity
        }
      }
    });

    // 3. 创建订单
    await tx.customerOrder.create({
      data: orderData
    });
  });
  ```

## 📦 依赖包清单

### 后端依赖
```json
{
  "@nestjs/common": "^10.0.0",
  "@nestjs/core": "^10.0.0",
  "@nestjs/platform-express": "^10.0.0",
  "@nestjs/passport": "^10.0.0",
  "@nestjs/jwt": "^10.0.0",
  "@prisma/client": "^5.0.0",
  "prisma": "^5.0.0",
  "class-validator": "^0.14.0",
  "class-transformer": "^0.5.1",
  "multer": "^1.4.5-lts.1",
  "sharp": "^0.33.0",
  "uuid": "^9.0.0"
}
```

**依赖说明**：
- `@nestjs/platform-express`: Express平台支持（文件上传）
- `@nestjs/passport` & `@nestjs/jwt`: JWT认证（已存在）
- `@prisma/client`: Prisma ORM客户端（已存在）
- `class-validator` & `class-transformer`: DTO验证（已存在）
- `multer`: 文件上传中间件
- `sharp`: 图片处理库（生成多尺寸图片、压缩、格式转换）⭐ 推荐
- `uuid`: 生成唯一文件名

### 前端依赖
无需额外依赖，使用原生JavaScript

**前端优化建议**（可选）：
- 使用 `lazysizes` 库实现图片懒加载
- 使用 `lozad` 库实现轻量级懒加载
- 使用原生 `loading="lazy"` 属性（现代浏览器支持）

## 🚀 部署步骤

1. 执行数据库迁移
2. 启动后端服务
3. 配置前端API地址
4. 测试所有功能
5. 上线发布

## 📋 实施检查清单

### 数据库
- [ ] 所有7张表创建成功
  - [ ] product（商品表，含位置字段）
  - [ ] product_category（分类表）
  - [ ] shopping_cart（购物车表）
  - [ ] customer_order（订单表）
  - [ ] order_item（订单明细表）
  - [ ] wishlist（心愿单表）
  - [ ] customer_address（收货地址表，新西兰格式）
- [ ] 索引创建正确
- [ ] 测试数据插入

### 后端API
- [ ] 所有接口开发完成
- [ ] 接口测试通过
- [ ] API文档完善

### 前端页面
- [ ] 商品列表展示正常
- [ ] 商品详情展示正常
- [ ] 购物车功能正常
- [ ] 订单功能正常
- [ ] 心愿单功能正常

### 后台管理
- [ ] 商品管理功能完整（列表、新增、编辑、删除、批量操作）
- [ ] 分类管理功能完整（树形结构、增删改）
- [ ] **商品位置管理功能完整** ⭐ 新增
  - [ ] 首页精选商品管理（最多4个）
  - [ ] 相关商品管理（最多4个）
  - [ ] 商品列表排序管理
  - [ ] 商品选择器功能正常
  - [ ] 位置保存功能正常
- [ ] 订单管理功能完整（列表、详情、状态修改、新西兰地址显示）
- [ ] 图片上传功能正常
- [ ] 富文本编辑器正常

### 测试
- [ ] 功能测试通过
- [ ] 性能测试通过
- [ ] 安全测试通过

---

## 📸 附录：图片尺寸完整对照表

### 生成的图片尺寸与用途对照

| 尺寸名称 | 尺寸（宽x高） | 裁剪方式 | 质量 | 用途 | 前端使用位置 |
|---------|-------------|---------|------|------|------------|
| **original** | 原图（最大1200px） | 保持比例 | 90% | 原图备份 | - |
| **large_800** | 800x877px | 保持比例 | 85% | 商品主图 | 详情页主图、列表页、首页Featured |
| **large_768** | 768x842px | 保持比例 | 85% | 大屏幕显示 | srcset中的768w |
| **medium_600** | 600x600px | 正方形裁剪 | 85% | 中等尺寸 | 桌面端商品列表、首页 |
| **medium_460** | 460x460px | 正方形裁剪 | 85% | 备用尺寸 | 备用 |
| **small_274** | 274x300px | 保持比例 | 85% | 移动端 | srcset中的274w |
| **thumbnail_150** | 150x150px | 正方形裁剪 | 80% | 缩略图 | 心愿单、购物车、比较列表 |
| **thumbnail_100** | 100x100px | 正方形裁剪 | 80% | 微缩图 | 详情页左侧缩略图列表 |
| **webp_600** | 600x600.webp | 正方形裁剪 | 80% | WebP格式 | 现代浏览器（体积小30-50%） |
| **webp_768** | 768x842.webp | 保持比例 | 80% | WebP格式 | 现代浏览器（体积小30-50%） |

### 前端页面使用场景详细说明

#### 1. 首页（index.html）- Featured Products区域
```html
<img
  src="large_800"
  srcset="large_800 800w, small_274 274w, large_768 768w"
  sizes="(max-width: 800px) 100vw, 800px"
/>
```
- **主要使用**: large_800 (800x877px)
- **响应式**: small_274 (移动端), large_768 (大屏幕)

#### 2. 商品列表页（shop/index.html）
```html
<img
  src="large_800"
  srcset="large_800 800w, small_274 274w, large_768 768w"
  sizes="(max-width: 800px) 100vw, 800px"
/>
```
- **主要使用**: large_800 (800x877px)
- **响应式**: small_274 (移动端), large_768 (大屏幕)

#### 3. 商品详情页（intelligent-marble-knife/index.html）- 主图区域
```html
<img
  src="large_800"
  data-large_image="large_800"
  srcset="large_800 800w, small_274 274w, large_768 768w"
  sizes="(max-width: 800px) 100vw, 800px"
/>
```
- **主要使用**: large_800 (800x877px)
- **响应式**: small_274 (移动端), large_768 (大屏幕)

#### 4. 商品详情页 - 左侧缩略图列表 ⭐ 新增
```html
<div
  data-thumb="thumbnail_100"
  data-thumb-srcset="thumbnail_100 100w"
  data-thumb-sizes="(max-width: 109px) 100vw, 109px"
>
```
- **主要使用**: thumbnail_100 (100x100px)
- **显示尺寸**: 约100-109px

#### 5. 心愿单/比较列表
```html
<img src="thumbnail_150" />
```
- **主要使用**: thumbnail_150 (150x150px)

#### 6. 购物车
```html
<img src="thumbnail_150" />
```
- **主要使用**: thumbnail_150 (150x150px)

#### 7. 后台管理 - 商品列表
```html
<img src="thumbnail_100" />
```
- **主要使用**: thumbnail_100 (100x100px)

### WebP格式使用示例

```html
<picture>
  <source
    type="image/webp"
    srcset="webp_600 600w, webp_768 768w"
    sizes="(max-width: 800px) 100vw, 800px"
  />
  <img
    src="large_800"
    srcset="large_800 800w, small_274 274w, large_768 768w"
    sizes="(max-width: 800px) 100vw, 800px"
    alt="Product Image"
    loading="lazy"
  />
</picture>
```

### 数据库存储示例

**mainImage字段**（TEXT类型，存储JSON字符串）：
```json
{
  "original": "uploads/product/202411/abc123.jpg",
  "large_800": "uploads/product/202411/abc123_800x877.jpg",
  "large_768": "uploads/product/202411/abc123_768x842.jpg",
  "medium_600": "uploads/product/202411/abc123_600x600.jpg",
  "medium_460": "uploads/product/202411/abc123_460x460.jpg",
  "small_274": "uploads/product/202411/abc123_274x300.jpg",
  "thumbnail_150": "uploads/product/202411/abc123_150x150.jpg",
  "thumbnail_100": "uploads/product/202411/abc123_100x100.jpg",
  "webp_600": "uploads/product/202411/abc123_600x600.webp",
  "webp_768": "uploads/product/202411/abc123_768x842.webp"
}
```

**images字段**（TEXT类型，存储JSON数组字符串）：
```json
[
  {
    "original": "uploads/product/202411/def456.jpg",
    "large_800": "uploads/product/202411/def456_800x877.jpg",
    "large_768": "uploads/product/202411/def456_768x842.jpg",
    "medium_600": "uploads/product/202411/def456_600x600.jpg",
    "medium_460": "uploads/product/202411/def456_460x460.jpg",
    "small_274": "uploads/product/202411/def456_274x300.jpg",
    "thumbnail_150": "uploads/product/202411/def456_150x150.jpg",
    "thumbnail_100": "uploads/product/202411/def456_100x100.jpg",
    "webp_600": "uploads/product/202411/def456_600x600.webp",
    "webp_768": "uploads/product/202411/def456_768x842.webp"
  },
  {
    "original": "uploads/product/202411/ghi789.jpg",
    ...
  }
]
```

### 文件系统存储结构

```
static/upload/product/
├── 202411/
│   ├── abc123.jpg                    (原图)
│   ├── abc123_800x877.jpg            (large_800)
│   ├── abc123_768x842.jpg            (large_768)
│   ├── abc123_600x600.jpg            (medium_600)
│   ├── abc123_460x460.jpg            (medium_460)
│   ├── abc123_274x300.jpg            (small_274)
│   ├── abc123_150x150.jpg            (thumbnail_150)
│   ├── abc123_100x100.jpg            (thumbnail_100)
│   ├── abc123_600x600.webp           (webp_600)
│   └── abc123_768x842.webp           (webp_768)
├── 202412/
│   └── ...
```

### 性能优化建议

1. **使用WebP格式**：体积减少30-50%，加载速度更快
2. **启用懒加载**：`loading="lazy"` 属性，延迟加载屏幕外图片
3. **使用CDN**：生产环境使用CDN加速图片加载
4. **响应式加载**：根据设备屏幕尺寸加载合适的图片
5. **图片压缩**：质量80-85%即可，肉眼几乎无差别
6. **缓存策略**：设置合理的浏览器缓存时间

---

**预计总工时：** 9-13天
**优先级：** 高
**风险评估：** 中等

